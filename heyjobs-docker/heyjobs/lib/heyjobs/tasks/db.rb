namespace :db do
  ...
  db_config_admin = db_config['development'].merge({'database' => 'postgres', 'schema_search_path' => 'public'})

  desc "Create the database"
  task :create do
    ActiveRecord::Base.establish_connection(db_config_admin)
    ActiveRecord::Base.connection.create_database(db_config['development']['database'])
    ActiveRecord::Base.connection.create_database(db_config['test']['database'])
  ...
  task :drop do
    ...
    ActiveRecord::Base.connection.drop_database(db_config['test']['database'])
  ...
  desc 'Apply default seeds to DB'
  task :seed do
    ActiveRecord::Base.establish_connection(db_config['development'])
    load('db/seeds.rb')
  end
